...
| web/lib + web/proxy.ts + web-config (open-next/wrangler) | partial | 2026-07-28 | eslint-config en twee `lib`-bestanden aangeraakt; **nog niet gereviewd**: `runtime-site-context.ts`, module-scope caches in `web/lib/supabase/service.ts` (module-level `cached` client), cache-keys per host/site_id, de fallback-keten |
