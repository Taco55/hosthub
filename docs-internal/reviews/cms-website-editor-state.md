...
| G3b | Padtabel + `locationOf` van statisch naar de template | console | todo | `_fieldPaths` (68 patronen) + `kDoc*` + `locationOf`/`_listLocationOf`/`_patternPrefixOf`/`_match` staan nog `static` op de repository; G3 bracht alleen de kaarten naar de instantie. **Aanpak vastgelegd na een mislukte poging:** de tabel is een *const literal* en kan dus geen instance-veld zijn — zet `kDoc*` en de tabel op **toplevel** in het domein (`_kChaletFieldPaths`), geef `WebsiteTemplate` een `final List<...> fieldPaths` via de constructor, en laat alleen de vier resolvers methodes worden. Een scripted in-class graft breekt de klasse (const_instance_field) — doe deze in één bewuste pass, niet met regelbereiken. `EditorFieldLocation`/`RowId`/`_Slot` moeten mee naar het domein, anders importeert domain→data |
