...
**Openstaande slice (niet blind gedaan):** de 25 React-Compiler-hookovertredingen in de boekingsflow.
Correct oplossen betekent effects en memoization herstructureren in `DateRangePicker`, `DateRangeModal`
en `useBookingState` — de code waarmee een gast daadwerkelijk boekt en betaalt. Die componenten hebben
géén tests, en ik kan de UI niet zelf doorlopen, dus dit hoort een eigen slice met tests-eerst te zijn
in plaats van een blinde refactor. Aparte instap: eerst de boekingslogica onder test (`useBookingState`
is zuivere logica en direct testbaar met de nieuwe suite), dán de hooks opschonen.
