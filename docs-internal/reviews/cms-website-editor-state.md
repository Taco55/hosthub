...
| G4 | Labels als data op kaart/rij i.p.v. 13 switches | console | deels | 7e92556 — **het positionele deel is weg**: `GroupListRow.fixedTitleLabels` vervangt de switch die de vijf transportkolommen op array-*index* labelde (S5 uit de review: "not data at all"). `LabelRef` (`String Function(dynamic s)`) is het patroon — een functieverwijzing, want intl_utils genereert `S`-members en een lookup op string overleeft codegen niet. Test eist dat élke rij die zijn titels vastzet ze ook benoemt. **Nog open:** de twaalf id-gesleutelde switches (cardTitle/cardSubtitle/cardIcon/fieldLabel/listTitle/…, ~145 cases). Die zijn minder erg — ze sleutelen op stabiele ids, niet op posities — maar een tweede template valt er nog doorheen naar `weCardContent`. Zelfde `LabelRef`-patroon, per kaart/rij gedeclareerd; puur mechanisch, dus een aparte pass waard |
...
## next_lens
RUN 8 loopt: G1, G2, G3 en G5 done. G3b done — **de structurele blokkade is weg**: kaarten én padtabel zitten per template. RUN 8: G1, G2, G3, G3b, G5, G6, G7 en G8 done; **G4 deels** (positionele labels weg, twaalf id-gesleutelde switches open). De structurele blokkade is opgeheven: kaarten, padtabel, media-routing en volgorde zitten allemaal per template. (labels als data), G6 (media-routing), G7 (`page`-kolom), G8 (adres-manifest).
