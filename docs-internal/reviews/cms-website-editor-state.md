...
**Coördinatie (2026-07-27 20:35):** een tweede actieve sessie ("Readiness review") werkt
ongecommit in `website_editor/**` + `web/**` (live draft-preview + padgebaseerde mapping —
fundament van fase 0) en heeft dit state-bestand hernoemd. Bericht gestuurd; console-fasen
(F0, F1, F3+) starten hier pas als dat werk gecommit/gestaakt is. De library-fase (F2, buiten
deze repo) gaat eerst en is door deze sessie geclaimd.
...
| F5 | Media: B11–B13 + `MediaLibraryCubit`, `cms_media` + Storage-RLS per site; beeldsleutels repo → site_config | console+supabase (+lib) | todo | — |
