...
| F5 | Media: B11–B13 + `MediaLibraryCubit`, `cms_media` + Storage-RLS per site; beeldsleutels repo → site_config | console+supabase (+lib) | in_progress | **Backend af** (console@8419890): bucket `site-media` met het pad als scope (`<site_id>/<uuid>.<ext>`, JO-patroon met `has_site_access`), writes = editor, API-reads scoped; migratie 20260727210000 lokaal toegepast; `supabase/tests/site_media_rls_test.sql` bewijst dat élke cross-site read én write geweigerd wordt (twee niet-admin owners, transactie rolt terug). Twee gaten gevonden: `cms_media` was publiek leesbaar (bestandsnamen/alt/gebruik van élke site) en beheer was owner-only terwijl de rest van het CMS editors toelaat. Dode JO-kopie in `supabase/policies/storage_policies.sql` (verwees naar `list_access`) vervangen. **Lib af** (sw@c75f8b8 = v0.12.0, gepusht): StyledMediaStrip, StyledSelectableGrid, StyledDropzone + StyledUploadRow + `media:`/`uploads:`-themegroepen; +14 tests, 915 groen. **Nog open:** `MediaLibraryCubit` + repository, de kiezer-modal, `MediaRow` in het schema wiren, beeldsleutels naar `site_config`, en de website die uit de bucket rendert |
| F6 | Publiceren: delta-tellers, per-pagina-uitklap (B14), `Openen` → taal+pagina+filter aan | console (+lib) | todo | — |
